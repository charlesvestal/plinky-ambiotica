...
/* harmony_process runs a fast_tanhf and a fractional delay read per sample PER VOICE and
 * measured 112-130us/block on device - the largest stage that was still executing from the
 * shared XIP cache, since looper/granular/microloop/dattorro are all already in SRAM.
 * PLINKY_DSP_RAM_FUNC un-inlines it and places its code in SRAM, exempting it from that
 * cache. Fallback so the desktop harness and older firmware still build.
 * NOTE: the SRAM code region is small and shared with the four stages above - if this stops
 * LINKING, this is the wrapper to drop, since it was the last one added. */
