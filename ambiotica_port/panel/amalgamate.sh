...
# Optional first arg = bisection stage (1..5). No arg = full panel.
STAGE="${1:-}"
LEVEL="${AMB_LEVEL:-}"       # chain complexity 0..6 (env); empty = full
if   [ -n "$LEVEL" ]; then OUT="$HN/plinky_ambiotica_L${LEVEL}.cpp"
elif [ -n "$STAGE" ]; then OUT="$HN/plinky_ambiotica_s${STAGE}.cpp"
...

{
    ...
    [ -n "$STAGE" ] && echo "#define AMB_STAGE $STAGE"
